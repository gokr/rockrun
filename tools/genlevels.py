...
'D' diamond, '@'/'2'/'3'/'4' player spawns (P1-P4), 'E' exit. Validation guarantees:
  - exact width/height per row
  - 1-4 player spawns and exactly one 'E'
  - spawn cells are clear and nothing may drop on them
...
def emit(name, section, g, needed, time_limit):
    ...
    spawns = [(x, y) for y, r in enumerate(rows)
              for x, c in enumerate(r) if c in SPAWNS]
    assert all(len(r) == w for r in rows), "row width mismatch"
    assert 1 <= len(spawns) <= 4, f"{name}: {len(spawns)} spawns (need 1..4)"
    assert sum(r.count(EXIT) for r in rows) == 1
    assert needed <= gems, f"{name}: needed {needed} > placed {gems}"
    # spawn cells must be distinct and nothing may sit directly above them
    assert len(set(spawns)) == len(spawns), (name, spawns)
    for (x, y) in spawns:
...
def level1():
    ...
    # extra spawn pockets on the floor for players 2-4 (cols 4, 6, 8)
    for sx, symbol in ((4, '2'), (6, '3'), (8, '4')):
        carve(g, sx, py, sx, py)
        g[py][sx] = symbol
    g[py][px] = START
...
def level2():
    ...
    carve(g, 2, 2, 7, 2)
    g[py][px] = START
    g[4][3] = '2'   # corridor floor
    g[5][2] = '3'   # pocket row
    g[6][3] = '4'   # corridor floor
    ...
    g[12][9] = 'f'
    g[16][39] = 'b'
    g[13][24] = 'b'   # grand hall
    g[19][18] = 'f'   # lower gallery
...
def level3():
    ...
    g[10][35] = 'f'
    g[21][24] = 'b'
    g[16][13] = 'b'   # twin vaults
    g[21][30] = 'f'   # lower mine
...
def level4():
    ...
    g[19][30] = 'f'
    g[20][49] = 'b'
    g[14][30] = 'b'   # grand hall
    g[24][32] = 'f'   # lower galleries
...
def level5():
    ...
    g[8][40] = 'b'
    g[22][28] = 'b'
    g[9][40] = 'f'    # upper chamber
    g[22][20] = 'b'   # lower mine
...
if __name__ == '__main__':
    ...
    target = os.path.join(here, '..', 'data', 'config', 'levels.ini')
